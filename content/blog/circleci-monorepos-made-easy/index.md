...
Right away, I will admit: CircleCI is not my favorite CI/CD tool.
...
However, sometimes, the correct choice isn't what we wa`n`t but what we have.
For my current engagement, the client had dozens of repositories successfully validating pull requests and deploying changes into their respective environments using CircleCI.
...
Nothing exotic here - for example, I might run a linter across all files and I might build and push an image for `subproject_one` to a particular Docker repository that is different from `subproject_two`.
...
However, in retrospect, I would not recommend it, and I migrated away from this approach.
It was finicky, error-prone, and difficult to maintain.
...
It _could_ become a huge mess of a file.

But!
I have identified a few techniques we can use to keep it modular, DRY (don't-repeat-yourself), and maintainable.
...
First, for every pull request and merge, I wanted to run some common tasks (such as validating the change has no syntax errors).

Second, when a change was merged to `main`, and had no git tag, I wanted to deploy it to a staging environment.

Third, when a change was merged to `prod` and had a tag of the form `v$.$.$` (such as`v1.0.0`), I wanted to deploy it to the production environment.
...
For my purposes, this was helpful to orchestrate a sequence of steps that required the usage of an API client given my deployment target did not have a CircleCI orb available.
